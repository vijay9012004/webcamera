...
.card { background: rgba(255,255,255,0.08); padding:20px; border-radius:20px; backdrop-filter: blur(12px); margin-bottom: 20px; }
        ...
        if self.model:
            ...
            if label=="drowsy":
                ...
                if time.time()-self.start_time>2:
                    self.alarm_on=True
                    if not self.alerted: self.alerted=True
            ...
            cv2.putText(img,f"{label.upper()} ({confidence:.1f}%)",(10,40),cv2.FONT_HERSHEY_SIMPLEX,1,color,2)
        else:
